...
<body>
	...
	<div class="navbar-fixed">
      ...
      <div class="login-logo">
        <b>INSpector System </b><br>HISTÓRICOS Follow de <?php echo $_SESSION['user']; ?>
      </div><!-- /.login-logo -->
      <div class="login-box-body">
        <p class="login-box-msg">En este apartado podrás visualizar tus Follows que ya estan dados de baja del sistema web.</p>
		        <form method="post">		          
		...
		if(isset($_POST['buscar'])){
			...
			if (mysqli_connect_errno()) {
                echo "<center><p style=\"color:#b40000\"><strong>Falló la conexion al Sistema LineINS.</strong></p></center>";                              
      				...
      				$sql = "SELECT * FROM `follow`.`casosF` WHERE `case` = '$buscar' and `status` = 0 and `inves` = '$us'";
                    $result = mysqli_query($conexion,$sql); 
                    if(mysqli_num_rows(mysqli_query($conexion,$sql)) == 0){ ?>
						<h4 class="red-text">No se ha encontrado el Follow solicitado. Verifica que el caso que solicitas te haya sido asignado y que ya esté dado de baja e intentalo nuevamente.</h4>
                    <?php }else{
